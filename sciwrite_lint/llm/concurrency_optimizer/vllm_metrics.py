"""vLLM Prometheus metrics fetcher (self-contained for portability).

Parses the ``/metrics`` endpoint exposed by a running vLLM
OpenAI-compatible server and returns a flat ``dict`` of key-value
pairs that the controller / monitor can consume without knowing about
Prometheus text format.

Why this lives inside ``concurrency_optimizer``: the only external
runtime dependency the dynamic controller has is "give me a
``fetch_metrics(endpoint) -> dict`` function". Bundling it here means
the package can be lifted into another vLLM project as a single
directory copy — no transitive imports out to a host project. The
host's own monitor / dashboard code can still import the same function
through this module if it wants the canonical implementation.

Tested against vLLM 0.5+ (cache_config:info gauge added) on
``Qwen3-8B-FP8-dynamic`` (text) and ``Qwen3-VL-8B-Instruct-FP8``
(vision).
"""
...
def _require_http_url(url: str) -> str:
    ...
    Guards ``urlopen`` against ``file://`` and other non-network schemes
    that could be injected via a misconfigured endpoint.
    """
...
_GAUGE_PATTERNS: list[tuple[str, str]] = [
    ...
    (
        rf"vllm:time_to_first_token_seconds_count\{{[^}}]*\}}\s+({_NUM})",
        "ttft_count",
    ),
    (rf"vllm:inter_token_latency_seconds_sum\{{[^}}]*\}}\s+({_NUM})", "itl_sum"),
    (
        rf"vllm:inter_token_latency_seconds_count\{{[^}}]*\}}\s+({_NUM})",
        "itl_count",
    ),
]
...
def fetch_metrics(endpoint: str) -> dict[str, float | int | str]:
    """Fetch all vLLM metrics from the Prometheus endpoint and ``/v1/models``.

    *endpoint* is the OpenAI-compatible base, e.g.
    ``http://localhost:5001/v1``.

    Returns a dict with keys like ``kv_cache_pct``, ``requests_running``,
    ``prefix_cache_hits``, ``e2e_latency_sum``, ``req_success_stop``,
    ``num_gpu_blocks``, ``block_size``, ``gpu_util_cap``, ``max_seq``.
